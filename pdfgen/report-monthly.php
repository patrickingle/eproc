...
<?php
	...
	$start = strtotime("01 " . getMonthName($_GET[month]) . " " . $_GET[year]);
	$end = strtotime("31 " . getMonthName($_GET[month]) . " " . $_GET[year]);
	...
	for($i=0; $i<count($tender); $i++) {
		...
		while($co = $conn->fetchObject($rq)) {
			if(strtoupper(getQuoteCurrency($tender[$i][tender_id])) == "USD") {
				$tender[$i][pr_value_usd] += $co->req_est_price;
				$total[pr][usd] += $co->req_est_price;
			}
			else if(strtoupper(getQuoteCurrency($tender[$i][tender_id])) == "IDR") {
				$tender[$i][pr_value_idr] += $co->req_est_price;
				$total[pr][idr] += $co->req_est_price;
			}
		}
	}
...
<body style="background: none;">
	...
	<table border="0" cellpadding="0" cellspacing="0" class="rfqTable fBorder" width="100%">
		...
		<tr>
			...
			<td valign="top" class="rightBorder bottomBorder"><?= $tender[$i][rfq_delivery] ?></td>
